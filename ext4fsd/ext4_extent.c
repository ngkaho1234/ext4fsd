...
#define EXT4_EXT_DATA_VALID			0x08		/* first half contains valid data */
...
static void ext4_ext_free_blocks(struct ext4_inode_ref *inode_ref,
				 ext4_fsblk_t block, __u32 count,
				 __u32 flags)
{
	ext4_balloc_free_blocks(inode_ref, block, count);
}

static __u16 ext4_ext_space_block(struct ext4_inode_ref *inode_ref)
{
	__u16 size;
	__u32 block_size = ext4_sb_get_block_size(&inode_ref->fs->sb);
...
static __u16 ext4_ext_space_block_idx(struct ext4_inode_ref *inode_ref)
{
	__u16 size;
	__u32 block_size = ext4_sb_get_block_size(&inode_ref->fs->sb);
...
static __u16 ext4_ext_space_root(struct ext4_inode_ref *inode_ref)
{
	__u16 size;
...
static __u16 ext4_ext_space_root_idx(struct ext4_inode_ref *inode_ref)
{
	__u16 size;
...
static __u16 ext4_ext_max_entries(struct ext4_inode_ref *inode_ref,
				     __u32 depth)
{
	__u16 max;
...
static __u32 ext4_ext_block_csum(struct ext4_inode_ref *inode_ref,
				    struct ext4_extent_header *eh)
{
	...
	if (ext4_sb_feature_ro_com(sb, EXT4_FRO_COM_METADATA_CSUM)) {
		__u32 ino_index = to_le32(inode_ref->index);
		__u32 ino_gen =
...
static void ext4_ext_drop_refs(struct ext4_inode_ref *inode_ref,
			       struct ext4_extent_path *path, __bool keep_other)
{
	int32_t depth, i;
...
static int ext4_ext_check(struct ext4_inode_ref *inode_ref,
			  struct ext4_extent_header *eh, __u16 depth,
			  ext4_fsblk_t pblk)
{
	struct ext4_extent_tail *tail;
	struct ext4_sblock *sb = &inode_ref->fs->sb;
...
static int ext4_find_extent(struct ext4_inode_ref *inode_ref, ext4_lblk_t block,
			    struct ext4_extent_path **orig_path, __u32 flags)
{
	struct ext4_extent_header *eh;
...
static int ext4_ext_insert_index(struct ext4_inode_ref *inode_ref,
				 ...
				 ext4_fsblk_t insert_block, __bool set_to_ix)
{
...
static inline __bool ext4_ext_can_prepend(struct ext4_extent *ex1,
...
static inline __bool ext4_ext_can_append(struct ext4_extent *ex1,
				       struct ext4_extent *ex2)
{
 ...
 */
static __bool ext4_ext_more_to_rm(struct ext4_extent_path *path, ext4_lblk_t to)
{
			     ...
			     ext4_lblk_t to)
{
	...
	__bool in_range = IN_RANGE(from, to_le32(path[depth].extent->first_block),
...
static int ext4_ext_split_extent_at(struct ext4_inode_ref *inode_ref,
				    struct ext4_extent_path **ppath,
				    ext4_lblk_t split, __u32 split_flag)
{
...
static int ext4_ext_convert_to_initialized(struct ext4_inode_ref *inode_ref,
					   struct ext4_extent_path **ppath,
					   ext4_lblk_t split, __u32 blocks)
{
...
int ext4_extent_get_blocks(struct ext4_inode_ref *inode_ref, ext4_lblk_t iblock,
			   __u32 max_blocks, ext4_fsblk_t *result,
			   __bool create, __u32 *blocks_count)
{
	struct ext4_extent_path *path = NULL;
	...
	if (ex) {
		...
		__u16 ee_len = ext4_ext_get_actual_len(ex);
